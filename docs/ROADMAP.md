...
- [x] Parameter info at caret to Claude - "Send Caret Context to Claude" walks the PSI element chain at the cursor (element, parent, grandparent, containing call/function) and injects the full code structure context so Claude sees exactly where the cursor is without description; shipped v0.8.44
