...
- [x] Branch and stash snapshot to Claude - "Send Branch Snapshot to Claude" injects the current branch, last 10 branches by commit date, and all stash entries via git subprocess; essential context before asking Claude to help with merges, rebases, or recovering lost work; shipped v0.8.47
- [x] Code folding map to Claude - "Send Folding Map to Claude" injects all collapsed regions in the current editor via `FoldingModel.allFoldRegions`; collapsed code is intentionally hidden - knowing what the developer folded tells Claude what they consider noise vs signal, shaping better suggestions (evidence: FoldingModel is IDE-only with no terminal equivalent; collapsed regions are never visible in source files; moat: high; effort: small); shipped v0.8.48
