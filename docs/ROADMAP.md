...
## Phase 14: Developer Awareness

The IDE knows what you're looking at right now, the full health of your project, and the state of your version control in a way no terminal snapshot can express. Ship the features that give Claude that real-time awareness.

- [x] Project health snapshot to Claude - "Send Project Health to Claude" aggregates error and warning counts across all open files via `DaemonCodeAnalyzerImpl.getFileLevelHighlights()` + `HighlightingSettingsPerFile`; gives Claude a project-wide severity map ("3 errors in PaymentService, 14 warnings in UserController") before any refactor conversation (evidence: project health is the most common "where do I start" question; DaemonCodeAnalyzer is IDE-only with live results; no terminal equivalent; moat: very high; effort: medium)
- [ ] Parameter info at caret to Claude - "Send Parameter Info to Claude" resolves the method call at the current cursor position via `ParameterInfoHandler` and injects the full signature with the currently highlighted parameter marked; lets Claude know exactly which argument you're trying to understand without any copy-paste (evidence: JetBrains shows this in the status bar on Ctrl+P - we surface it as Claude context; PSI resolution is IDE-only; moat: very high; effort: small)
- [ ] Inlay hints at caret to Claude - "Send Inlay Hints to Claude" injects the IDE's inferred type annotations at and around the caret from `InlayHintsProvider`; exposes Kotlin/Java type inference results that never appear in source text - exactly what Claude needs to reason about inferred types (evidence: Cursor surfaces inferred types in chat; InlayHints API is IDE-only, not in source files; moat: very high; effort: medium)
- [ ] Full file history to Claude - "Send File History to Claude" injects the complete commit log for the current editor file via `VcsHistoryProvider.createSessionFor()`; unlike Phase 12's selection history (line-range only), this covers all commits that touched the file with author/date/message - answers "how did this file evolve" before a large refactor (evidence: `git log -- <file>` is terminal-doable but requires switching context; VcsHistoryProvider works across all VCS not just git; moat: medium-high; effort: small)
- [ ] Branch and stash snapshot to Claude - "Send VCS State to Claude" injects the current branch, all local/remote branches, and the stash list via `GitBranchUtil.getLocalBranches()` + `GitStashUtils.loadStashStack()`; gives Claude the full VCS context before "should I branch this work" or "what's in my stash" conversations (evidence: `git branch -a && git stash list` is terminal-doable but requires switching windows; GitBranchUtil is richer and IDE-integrated; moat: medium-high; effort: small)
- [ ] Code folding map to Claude - "Send Folding Map to Claude" injects all collapsed regions in the current editor via `FoldingModel.allFoldRegions`; collapsed code is intentionally hidden - knowing what the developer folded tells Claude what they consider noise vs signal, shaping better suggestions (evidence: FoldingModel is IDE-only with no terminal equivalent; collapsed regions are never visible in source files; moat: high; effort: small)
