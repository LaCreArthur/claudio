...
# 3. Restarts Rider
# 4. Opens the Claudio panel (unless --no-open-gui)
...
PLUGIN_DIR="$RIDER_CONFIG_DIR"/plugins/claudio
OPEN_GUI=true
...
rm -rf "$PLUGIN_DIR"
unzip -o build/distributions/claudio-*.zip -d ~/Library/Application\ Support/JetBrains/Rider2025.3/plugins/
...
if [ "$OPEN_GUI" = true ]; then
  ...
  sleep 1
  osascript -e 'tell application "System Events" to keystroke "Claudio"'
  sleep 0.5
